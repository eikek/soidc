...
class JWETest extends FunSuite with Syntax:
  ...
  test("init vector"):
    assertEquals(Base64String.encode(data.initVector), Base64String.unsafeOf("48V1_ALb6US04U3b"))

  test("encrypt/decrypt cek"):
    val decRfc = Encrypt.decryptCEK1(data.contentEncKeyEncrypted.decoded.toArray, data.privateKey).value
    assertEquals(decRfc, data.contentEncKeySpec)
    val encBytes = Encrypt.encryptCEK256(data.contentEncKeySpec, data.publicKey).value
    val decKey = Encrypt.decryptCEK256(encBytes.toArray, data.privateKey).value
